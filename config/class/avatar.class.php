<?php
  ...
  class Avatar{
    ...
    public function copyAvatar($og_file, $when, $grp){
      ...
      if ($when == "user") {
        $to = "../../users/$session/avatar/Stomigram_".$new_name;
      } else if ($when == "group") {
        $to = "../../group/$grp/Stomigram_".$new_name;
    ...
    public function cropAvatar($when, $grp){
      ...
      if ($ext == "gif"){
        if ($when == "user") {
          ...
          imagegif($dst_image, "../../users/$session/avatar/Stomigram_".$new_name.".gif");
          return "$session/avatar/Stomigram_".$new_name.".gif";

        } else if ($when == "group") {
          self::deleteAvatars("group", $grp);
          imagegif($dst_image, "../../group/$grp/Stomigram_".$new_name.".gif");

          $query = $this->db->prepare("UPDATE groups SET grp_avatar = :name WHERE group_id = :grp");
          $query->execute(array(":name" => $new_name.".gif", ":grp" => $grp));

          return "$grp/Stomigram_".$new_name.".gif";
        }

      } else if($ext =="png"){
        if ($when == "user") {
          self::deleteAvatars("user", "");
          imagepng($dst_image, "../../users/$session/avatar/Stomigram_".$new_name.".png");
          return "$session/avatar/Stomigram_".$new_name.".png";

        } else if ($when == "group") {
          self::deleteAvatars("group", $grp);
          imagepng($dst_image, "../../group/$grp/Stomigram_".$new_name.".png");

          $query = $this->db->prepare("UPDATE groups SET grp_avatar = :name WHERE group_id = :grp");
          $query->execute(array(":name" => $new_name.".png", ":grp" => $grp));

          return "$grp/Stomigram_".$new_name.".png";
        }

      } else {
        if ($when == "user") {
          self::deleteAvatars("user", "");
          imagejpeg($dst_image, "../../users/$session/avatar/Stomigram_".$new_name.".jpg");
          return "$session/avatar/Stomigram_".$new_name.".jpg";

        } else if ($when == "group") {
          self::deleteAvatars("group", $grp);
          imagejpeg($dst_image, "../../group/$grp/Stomigram_".$new_name.".jpg");

          $query = $this->db->prepare("UPDATE groups SET grp_avatar = :name WHERE group_id = :grp");
          $query->execute(array(":name" => $new_name.".jpg", ":grp" => $grp));

          return "$grp/Stomigram_".$new_name.".jpg";
        }
