    ...
    public function explorePhotos(){
      ...
      if ($query->rowCount() > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          echo "<div class='exp_finds_ph inst'>
            <div class='exp_f_ph_img'>
              <img src='{$this->DIR}/media/Stomigram_{$image}' alt='' data-imgby='{$universal->GETsDetails($user, "username")}' data-postid='{$post}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}' class='{$filter}'>
            </div>
        ...
        echo "<div class='home_last_mssg exp_p_last'>
          <img src='{$this->DIR}/images/needs/st.jpg'>
