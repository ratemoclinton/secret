  ...
  class group{
    ...
    public function grpAvatar($grp){
      ...
      if ($avatar == "") {
        ...
        return "{$this->DIR}/group/$grp/Stomigram_$avatar";
      }
    }
    ...
    public function getGrpPhotos($grp){
      ...
      if ($count == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          <img src='{$this->DIR}/media/Stomigram_{$image}' alt='' data-imgby='{$universal->GETsDetails($userid, "username")}' data-postid='{$post_id}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}' class='{$filter}'></div>";
        }
      }
    }
    ...
    public function dltGrp($grp){
      ...
      if ($av != "") {
        unlink("../../group/{$grp}/Stomigram_$av");
      }
