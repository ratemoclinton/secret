  ...
  class post{
    ...
    public function imagePost($file, $value, $tags, $font, $loc, $filter, $when, $grp){
      ...
      if (in_array($ext, $allowed)) {
        if ($error == 0) {
          $new_name = time().".".$ext;
          if (move_uploaded_file($tmp_name, "../../media/Stomigram_".$new_name)) {
      ...
      if ($type == "text") {
        ...
        while ($trow = $tquery->fetch(PDO::FETCH_OBJ)) {
          ...
          return
            ...
            <img src='{$this->DIR}/media/Stomigram_{$img}' alt='Stomigram_{$img}' class='p_img {$filter}' data-postid='{$post_id}' data-imgby='{$u}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}'>";
        }
      } else if ($type == "video") {
          ...
          $text = $universal->toAbsURL($text);
           return
             "<div class='p_abt e ". self::addMoreClass($text) ."' style='font-size: {$size}px' spellcheck='false'><p>$text</p></div>
              ...
              <div class='post_marginer'></div>
              <div class='p_vid'>
              <video src='{$this->DIR}/media/Stomigram_{$vid}' loop preload='auto'></video>
    ...
    public function getPhotosPost($id){
      ...
      if ($count == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          <img src='{$this->DIR}/media/Stomigram_{$image}' alt='' data-postid='{$post_id}' data-imgby='{$r}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}' class='p_pho_img {$filter}'></div>";
        }
      ...
      if ($count == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          echo "<div class='p_vid video_vid user_post_vid'>
            <video src='{$this->DIR}/media/Stomigram_{$video}' loop preload='auto'></video>
            <span class='p_vid_pp_large'><i class='material-icons'>play_arrow</i></span>
            <span class='p_vid_cur p_vid_time_teaser'>0:00</span>
            <span class='p_vid_time_bubble'>0:00</span><div class='p_vid_ctrls'><div class='p_vid_seek'>
            <input class='p_vid_seek_range' type='range' name='p_vid_range' value='0' min='0' max='100' step='1'>
          ...
          if ($type == "text") {
            ...
            if ($imgquery1->rowCount() > 0) {
              while ($imgrow = $imgquery1->fetch(PDO::FETCH_OBJ)) {
                ...
                if(file_exists("../../media/Stomigram_{$image}")){
                  unlink("../../media/Stomigram_$image");
                }
            ...
            if ($vidquery1->rowCount() > 0) {
              while ($vidrow = $vidquery1->fetch(PDO::FETCH_OBJ)) {
                $video = $vidrow->video;
                if(file_exists("../../media/Stomigram_{$video}")){
                  unlink("../../media/Stomigram_$video");
                }
