  ...
  if (!((type == allowed[0]) || (type == allowed[1]) || (type == allowed[2]))) {
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('.send_mssg_before').fadeOut(50);
        elem.val('');
        var ht = "<div class='m_m_divs my_mm_div'><div class='m_m my_mm'><img src='"+ DIR +"/message/Stomigram_"+ data.m +"' class='m_m_img'></div><span class='m_m_time'>Just now</span></div>";
        $('.mssg_helper').before(ht);
        $('.m_m_wrapper').animate({scrollTop: 10000000}, 500);
        $('.m_m_img').imageShow();
      }
  ...
  if (!((type == allowed[0]) || (type == allowed[1]) || (type == allowed[2]))) {
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('.send_mssg_before').fadeOut(50);
        $('.overlay-2').hide();
        $('.sli_avatar_img').prop('src', DIR+'/grp_mssg_avatar/Stomigram_'+data.grp_av);
        $('.mssg_left').find('#cgrp_'+grp).find('img').prop('src', DIR+'/grp_mssg_avatar/Stomigram_'+data.grp_av);
